...
$get_data= $db->sellTotalMaskTotalByDate($data['startDate'],$data['endDate']);
...
if(empty($get_data)){
    ...
    $db->saveApiResult('sellTotalMaskTotalByDate',$request_data,$response_data,$api_status);
...
function check_sort_data($request){
    ...
    if(!CheckDateTime($data['endDate'])){
        $return['msg'] = 'Wrong endDate formate.';
        return $return;
    }
